...
import { getRiskBasel3WithSign } from './RiskLiquidityACTUSVerifierTest_basel3_WithsignUtils.js';


async function main() {
   const userLiquidityThreshold_LCR = parseFloat(process.argv[2]);   
   const url = process.argv[3];
   const proof = await getRiskBasel3WithSign(userLiquidityThreshold_LCR, url);
